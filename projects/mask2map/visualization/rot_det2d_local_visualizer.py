...
import cv2
import torch
from .det2d_palette import _get_adaptive_scales, get_palette, jitter_color
# from ..structures.bbox import QuadriBoxes, RotatedBoxes
from mmdet.structures.mask import BitmapMasks, PolygonMasks, bitmap_to_polygon
from .det2d_local_visualizer import DetLocalVisualizer
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'bboxes' in instances:
            ...
            if isinstance(bboxes, RotatedBoxes):
                centers = bboxes.centers
                # x, y, w, h, a
                half_widths = bboxes.tensor[:, 2] * 0.5
                angles = bboxes.tensor[:, 4]
                ...
                x_ends = x_starts + half_widths * cos_a
                y_ends = y_starts + half_widths * sin_a
                
                x_data = torch.stack([x_starts, x_ends], dim=1).numpy()
                y_data = torch.stack([y_starts, y_ends], dim=1).numpy()
                
                self.draw_lines(x_data, y_data, colors=colors, line_widths=self.line_width)
